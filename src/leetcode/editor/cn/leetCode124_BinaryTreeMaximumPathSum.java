...
class Solution {
    ...
    public int sum(TreeNode root){
        ...
        int left_maxValue = Math.max(sum(root.left), 0);
        int right_maxValue = Math.max(sum(root.right), 0);

        int priceNewPath = root.val + left_maxValue + right_maxValue;

        maxSum = Math.max(maxSum,priceNewPath);

        return root.val + Math.max(left_maxValue,right_maxValue);
    }
